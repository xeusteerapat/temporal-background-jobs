...
export async function queryApplicationData(
...
export async function callPaymentService(
	...
	try {
		...
		logger.info('Payment service response received', {
...
export async function callDocumentService(
	...
	const logger = Context.current().log;
	logger.info('Calling document service', {
	...
	try {
		...
		logger.info('Document service response received', {
			success: response.data.success,
		});
		return response.data;
	} catch (error) {
		logger.error('Document service call failed', {
			error: error instanceof Error ? error.message : 'Unknown error',
		});
