...
export async function applicationProcessingWorkflow(
	input: WorkflowInput
): Promise<string> {
	...
	try {
      ...
      `,
		...
		log.info('Application processing workflow completed successfully', {
			applicationId,
		});

		return `Application ${applicationId} processed successfully`;
	} catch (error) {
		log.error('Application processing workflow failed', {
			applicationId,
