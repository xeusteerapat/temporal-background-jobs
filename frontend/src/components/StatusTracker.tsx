		...
		const pollStatus = async () => {
			try {
				...
				if (response.success && response.application) {
					...
					setLastUpdated(
						new Date(response.application.updatedAt).toLocaleString()
					);
					setError('');

					// Stop polling if completed or failed
					if (
						response.application.status === 'completed' ||
				...
				{status === 'processing' && (
					<div className='bg-blue-50 p-3 rounded-md'>
						<p className='text-blue-800 text-sm'>
							Your application is being processed. This includes payment
							processing, document generation, and email notifications.
						</p>
					</div>
				)}
				...
				{status === 'completed' && (
					<div className='bg-green-50 p-3 rounded-md'>
						<p className='text-green-800 text-sm'>
							✅ Application processing completed successfully! Check your email
							for documents and confirmation.
						</p>
					</div>
				)}
				...
				{status === 'failed' && (
					<div className='bg-red-50 p-3 rounded-md'>
						<p className='text-red-800 text-sm'>
							❌ Application processing failed. Please contact support or try
							again.
						</p>
					</div>
				)}
